# Код представляет собой веб-приложение Streamlit, 
# которое отображает данные с датчиков влажности почвы и позволяет пользователю контролировать полив почвы.
# Функции table_to_df, session, data_update и watering импортируются из основного модуля. Модули json и streamlit также импортируются.

from main import table_to_df, session, data_update, watering
import streamlit as st
import json

# Определена основная функция, которая задает заголовок веб-страницы, считывает данные из файла JSON, 
# задает конфигурацию веб-страницы и отображает данные с датчиков влажности почвы в пользовательском интерфейсе.
# Данные о влажности почвы отображаются с помощью компонентов Streamlit line_chart и dataframe. 
# Пользователь также может включать / отключать полив почвы с помощью кнопок, 
# и функция полива вызывается с соответствующими аргументами.
# Когда пользователь нажимает кнопку "Обновить данные", 
# вызывается функция experimental_rerun модуля Streamlit для обновления данных, отображаемых на веб-странице.
def main():
    st.set_page_config(
        page_title="Soil",
        layout="wide"
